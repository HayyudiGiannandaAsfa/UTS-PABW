...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE contacts SET id = ?, name = ?, email = ?, password = ?, created = ? WHERE id = ?');
        $stmt->execute([$id, $name, $email, $password, $created, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <input type="text" name="password" placeholder="Password" value="<?=$contact['password']?>" id="password">
